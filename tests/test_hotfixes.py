...
class HotfixTests(unittest.TestCase):
    ...
    def test_management_package_keeps_closed_debt_and_later_payment(self):
        ...
        self.assertIn("def registrar_movimiento_pago_posterior_gasto", source)
        self.assertIn("Pago posterior gasto compartido", source)
        self.assertIn("i.estado = 'pendiente'", source)
        self.assertIn("movimiento_id INTEGER", source)
        self.assertIn("SET movimiento_id = %s", source)
        self.assertIn("estado_visual_gasto_compartido_item", source)
        self.assertIn("Mov. caja #", template)
        self.assertIn("Registrar pago", template)
        self.assertIn("Cargar comprobante posterior", template)
        self.assertIn("Aceptar posterior", template)
        self.assertIn("registrar_pago_posterior_comprobante", source)
        self.assertIn("pod&eacute;s enviar el comprobante", portal)
        self.assertNotIn("Contact&aacute; a administraci&oacute;n para regularizarlo", portal)
    ...
    def test_manual_app_notifications_admin_screen_is_present(self):
        ...
        self.assertIn("Diagn&oacute;stico", template)
        self.assertIn("data-pwa-permission-status", template)
        self.assertIn("diagnostico_push", source)
        self.assertIn("updatePwaDiagnostics", (Path(app.__file__).parent / "static" / "app.js").read_text(encoding="utf-8-sig"))
        ...
        self.assertNotIn("Tests registrados", portal)
